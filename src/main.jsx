...
function HomeScreen({ activePerson, setActivePerson, primaryMemberName, familyList, items, notifications, onMarkNotificationAsRead, onAdd, onViewTimeline, onOpenRecord, showNotifications, showReadNotifications, setShowReadNotifications, unreadNotificationPage, setUnreadNotificationPage, readNotificationPage, setReadNotificationPage, toggleNotifications }) {
  ...
  // Always show primary account holder initials/profile icon, not the selected person
  const initials = primaryMemberName ? primaryMemberName.split(' ').map(part => part[0]).slice(0, 2).join('').toUpperCase() : '?';
  ...
  return <>
    ...
    <div className={`person-select ${personMenuOpen ? 'person-open' : ''}`}>
      <button className="person-picker" onClick={() => setPersonMenuOpen(open => !open)} aria-expanded={personMenuOpen}><span className="person-mini">{initials}</span><span><b>{activePerson || 'No profile yet'}</b><small>{isPrimaryHolderActive ? 'Personal health space' : (familyList.find(m => m.name === activePerson)?.relationship || 'Family member')}</small></span><ChevronDown size={18}/></button>
